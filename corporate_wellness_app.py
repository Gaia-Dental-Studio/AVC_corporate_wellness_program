...
import pandas as pd
from model import ModelCorporateWellness  # Importing the Model class from model.py
import numpy as np

def app():

    # Load treatment prices CSV to get the list of treatments
    treatment_prices_df = pd.read_csv(r'corporate_wellness_data\treatment_prices.csv')
    treatment_list = treatment_prices_df['Treatment'].tolist()

    treatment_cost_df = pd.read_csv(r'corporate_wellness_data\treatment_costs.csv')

    # Load DSP CSV
    dsp_df = pd.read_csv(r'corporate_wellness_data\dsp.csv')
